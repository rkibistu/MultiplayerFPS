...
public class AudioSetup {
    ...
    public AudioSetup(AudioSetup other) {

        CopyFrom(other);
        Debug.Log("Copied! Clips: " + Clips.Count());
    }
...
public sealed class AudioEffect : MonoBehaviour {
    ...
    private void PlayClip(int clipIndex) {
        ...
        _audioSource.pitch = _currentSetup.Pitch + UnityEngine.Random.Range(-_currentSetup.MaxPitchChange, _currentSetup.MaxPitchChange);
    ...
    private int NextClipIndex(AudioSetup setup) {
        ...
        int clipIndex = UnityEngine.Random.Range(0, setup.Clips.Length);
